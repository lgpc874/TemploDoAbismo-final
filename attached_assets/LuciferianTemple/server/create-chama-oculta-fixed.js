...
async function createChamaOculta() {
  ...
  try {
    // Primeiro, deletar o grimório que foi criado sem capítulos
    await supabase
      .from('grimoires')
      .delete()
      .eq('title', '📗 A Chama Oculta – Um Nome Que Não Te Ensinaram');
    ...
    // Criar os capítulos com os campos corretos
    const chapters = [
      {
    ...
    if (chaptersError) {
      ...
      console.log(`✅ ${chapters.length} capítulos criados com sucesso!`);
    }
