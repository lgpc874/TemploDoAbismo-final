...
async function recreateMentiraDoPecado() {
  try {
    console.log('🔥 Recriando "A MENTIRA DO PECADO – O PESO QUE NUNCA FOI TEU"...');

    // Buscar grimório ID 30
    ...
    // Primeiro, deletar todos os capítulos existentes
    await supabase
      .from('chapters')
      .delete()
      .eq('grimoire_id', 30);

    console.log('🗑️ Capítulos antigos deletados');

    // Criar os capítulos seguindo o script original
    const chapters = [
      {
...
<p>Que estas palavras marquem não apenas o fim de um ciclo de escravidão, mas o início de uma jornada de soberania. Livre, soberano, e finalmente pronto para descobrir o que significa voar sem correntes.</p>`
    ...
    console.log('\n🔥 Grimório "A MENTIRA DO PECADO" recriado com sucesso!');
